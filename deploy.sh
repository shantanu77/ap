...
$SSH "cd /var/www/ap && git pull origin main && npm ci --production=false && set -a && source .env.production && set +a && npx prisma migrate deploy && npm run build && pm2 restart ap"
